...
from quran_backend.legal.models import PrivacyPolicy
...
We collect anonymous usage analytics to improve the Quran Backend API and provide a better user experience. The data we collect includes:

- **Feature Usage Events**: Which features you use (e.g., Surah viewed, reciter played, translation selected, bookmark created)
- **Session Information**: Anonymous session IDs to track user flows
- **Performance Metrics**: API response times and error rates
- **Country-Level Location**: Your country code (2-letter ISO code) derived from your IP address - we do NOT store city, coordinates, or IP addresses
...
- User-generated content (search queries are anonymized - only query length tracked)

## User Identification

All user IDs are hashed using SHA-256 with a secret key salt, making them irreversible and unlinkable to your actual account.

## Data Retention

- **Analytics Events**: Automatically deleted after 90 days
- **Aggregate Metrics**: Retained indefinitely with no user linkage
- **Your Data Deletion Right**: You can request immediate deletion of all your analytics data via the `/api/v1/analytics/delete-my-data/` endpoint
...
For privacy questions or data deletion requests, please contact: privacy@quranbackend.example.com
